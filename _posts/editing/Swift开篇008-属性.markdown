...
<font size=5>**PART_C 计算属性(-案例值得琢磨-)**
...
<font size=5>**PART_F 类型属性(`static`)：定义某个类型所有实例共享的数据**
