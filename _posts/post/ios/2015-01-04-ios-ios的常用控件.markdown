...
temp-layout:       post
temp-title:        "ios的常用控件"
subtemp-title:     "包含TextField、Switch、Slider、WebView等常用控件的使用介绍"
...
2. **Progress-使用 Timer 控制进度条的进度**   
	...
	func download() {
		...
		if pv_test.progress == 1.0 {
			mTimer.invalidate()
			var alert = UIAlertView(temp-title: "下载完成", message: "下载结束哦，哥哥", delegate: nil, cancelButtontemp-title: "OK")
			alert.show()
			return
		}
	}
...
- **Alert**
    ...
    ``` swift
    // 单个选项
    UIAlertView(temp-title: "警告", message: "我是警告内容", delegate: nil, cancelButtontemp-title: "知道").show()
    
    // 两个选项
    UIAlertView(temp-title: "警告", message: "我是警告内容", delegate: nil, cancelButtontemp-title: "取消", otherButtonTitles: "确定").show()
    
    // 多个选项...
    UIAlertView(temp-title: "警告", message: "我是警告内容", delegate: nil, cancelButtontemp-title: "取消", otherButtonTitles: "确定", "玩一玩").show()
    ```
    
- **ActionSheet**    

    ``` swift
    var asTest: UIActionSheet = UIActionSheet(temp-title: "选择列表", delegate: nil, cancelButtontemp-title: "取消", destructiveButtontemp-title: "破坏性选项", otherButtonTitles: "百度", "腾讯", "阿里")
...
- **ActionControl(掌握这个可以了)**
    ...
    ```	swift	
    var acTest: UIAlertController = UIAlertController(temp-title: "标题", message: "消息内容", preferredStyle: UIAlertControllerStyle.alert)
    
    acTest.addAction(UIAlertAction(temp-title: "取消", style: UIAlertActionStyle.cancel) {(alertAction) -> Void in NSLog("取消")})
    acTest.addAction(UIAlertAction(temp-title: "确定", style: UIAlertActionStyle.default) {(alertAction) -> Void in NSLog("确定")})
    ...
    acTest.addAction(UIAlertAction(temp-title: "取消", style: UIAlertActionStyle.cancel) {(alertAction) -> Void in NSLog("取消")})
    acTest.addAction(UIAlertAction(temp-title: "破坏性选项", style: UIAlertActionStyle.destructive) {(alertAction) -> Void in NSLog("破坏")})
    acTest.addAction(UIAlertAction(temp-title: "百度", style: UIAlertActionStyle.default) {(alertAction) -> Void in NSLog("百度")})
    acTest.addAction(UIAlertAction(temp-title: "腾讯", style: UIAlertActionStyle.default) {(alertAction) -> Void in NSLog("腾讯")})
    acTest.addAction(UIAlertAction(temp-title: "阿里", style: UIAlertActionStyle.default) {(alertAction) -> Void in NSLog("阿里")})
